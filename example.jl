...
save_diagram_path = joinpath(save_diagram_dir, "Diagram.png")
save_measures_path = joinpath(save_measure_dir, "Measure.png")
peach_fuzz_color = RGB(1.0, 0.745, 0.596)
mintgreen_color = RGB(0.7333333333333333, 0.3803921568627451, 0.6784313725490196)
max_dim = 1

x, y = load_data(data_path)
save_scatter_plot(x, y, save_data_path)
...
for k in sgnl
    ...
    for i in 1:size(reps[k],2)
        Plots.plot!(data[1, [reps[k][1, i], reps[k][2, i]]], data[2, [reps[k][1, i], reps[k][2, i]]], linecolor = color_vec[k], linewidth = 4.0, xlabel = "x", ylabel = "y")
    end
end

save_mapping_path = joinpath(save_mapping_dir1, "Mapping.png")
savefig(save_mapping_path)

x = collect(range(0, stop = maximum(bars[:,2]), length = 2000))
cent5 = hcat([plot_centrality5(j, x, time, bars, pers, class, 1, 1) for j in 1:length(pers)]...);
y = [maximum(cent5[:, i]) for i in 1:size(cent5, 2)]
sgnl1 = findall(x -> x >= 7, y)
...
for k in sgnl1
    ...
    for l in 1:size(reps[j],2)
        Plots.plot!(data[1, [reps[j][1, l], reps[j][2, l]]], data[2, [reps[j][1, l], reps[j][2, l]]], linecolor = color_vec[k], linewidth = 4.0, xlabel = "x", ylabel = "y")
    end   
end

save_mapping_path1 = joinpath(save_mapping_dir2, "Mapping.png")
...
for k in sgnl1
    ...
    Plots.plot!(cycl[1,:], cycl[2,:], seriestype = :scatter, legend = false, markersize = 5, markercolor = color_vec[k])
    for i in 1:size(reps[k],2)
        plt = Plots.plot!(data[1, [reps[k][1, i], reps[k][2, i]]], data[2, [reps[k][1, i], reps[k][2, i]]], linecolor = color_vec[k], linewidth = 4.0, xlabel = "x", ylabel = "y")
    end
end

save_mapping_path2 = joinpath(save_mapping_dir3, "Mapping.png")
